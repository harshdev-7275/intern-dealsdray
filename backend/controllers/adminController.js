...
const addEmployee = asyncHandler(async(req, res)=>{
    const {name, email, mobNumber, designation, gender, course, image} = req.body;
    console.log(req.body);
    if(!name || !email || !mobNumber || !designation || !gender || !course || !image){
        res.status(400);
        throw new Error("All fields are mandatory");
    }
    ...
    const employee = await Employee.create({name, email, mobNumber, designation, gender, course, image});
    if(employee){
...
export {createAdmin, loginAdmin, addEmployee, updateEmployee, deleteEmployee, getAllEmployees}